...
# TV Shows directory
$TvShowDir = "/Directory/to/tv/shows"
 

# Movies directory
$MovieDir = "/Directory/to/movies"

# HandBreakCLI Directory (Wherever you extracted Handbreak to.  
Example: /Downloads/HandBrakeCLI-1.0.3-win-x86_64
$HandBreakDir = "/snap/handbrake-jz/134/bin"
...
if(-not(Test-Path("$HandBreakDir\HandBrakeCLI"))){
    Write-Host "HandBrakeCLI not found in $HandBreakDir Please make sure that HandBreak is installed.  Quitting" -ForegroundColor Red
...
foreach($File in $AllLargeFiles){
    ...
    if(-not($HashTable.ContainsKey("$FinalName"))){
        ...
        #& $HandBreakDir\HandBrakeCLI -i "$InputFile" -t 1 --angle 1 -o "$OutputFile" -f $FileFormat --modulus 2 -e x265 -q 23 --cfr -a 1 -E copy:* -6 dpl2 -R 48 -B 64 -D 0 --gain 0 --audio-fallback ac3 -m --encoder-preset=veryfast --verbose=1 2> "$LogFileDir\$LogEpisodeName.txt"
        & $HandBreakDir\HandBrakeCLI -i "$InputFile" -t 1 -o "$OutputFile" -f $FileFormat --modulus 2 -e x265 -q 23 --cfr --audio-lang-list $Language -E copy:ac3 -6 dpl2 -R 48 -B 256 --audio-fallback ac3 -m --encoder-preset=fast --verbose=1 --subtitle-lang-list $Language --subtitle 'scan' --subtitle-forced --subtitle-burned 2> "$LogFileDir\$LogEpisodeName.txt"
